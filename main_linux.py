...
PMD_SETTINGS = {
    'port': '/dev/ttyUSB0',  # Linux port format
    'baudrate': 115200,
    'bytesize': 8,
    'stopbits': 1,
    'timeout': 1,
...
# Configuration flags
LIST_ALL_PORTS = True  # Set to True to list all available COM ports
...
def get_cpu_usage(process_name: str) -> float:
    ...
    for process in psutil.process_iter(['pid', 'name']):
        if process.info['name'].lower() == process_name.lower():  # Handle case sensitivity
...
def save_data_to_csv(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        file_path = os.path.join('data', f'{date_name}_power_measurements.csv')  # Use os.path.join for cross-platform paths
